...
\title{T-61.3050 Term Project, final report \\ Single-character OCR using Support Vector Machines}
...
training set of 42152 correctly labeled samples using k-fold cross-validation with k=20
\cite{training_set}.
...
solution to our problem, as the data points would still have to be labeled in the dimensionally
...
without supervision.  We had pre-labeled training data at our disposal, however, so we could make
...
unlabeled sample, the k nearest neighbors are looked up (using either simple Euclidean distance, or
...
\cite{libsvm_guide,svm_chemistry}. It has several desirable properties.  Firstly, it is numerically
...
function, RBF had desirable properties in that regard as well.  These are discussed further in
...
is, picked from the entire set of labeled data available) can greatly affect the resulting
...
K-fold cross-validation avoids these issues by dividing the labeled data into k-segments of equal
...
primary classifier performed better than the classifier tree.  In the end, this was both desirable
and intuitive.  It was desirable because it makes our solution simpler, and simple solutions are
easier to sanity-check.  It was intuitive (especially in retrospect) because our solution was in
fact analogous to how multi-class SVM's are often constructed internally anyway (as discussed in
...
90.8\% against the given training set of 42152 correctly labeled samples, as measured by k-fold
cross-validation with k=20.  On a 2.1GHz Xeon development machine, a full run of the training and
...
algorithm) have been contrasted with the correctly labeled data.}
...
selected from the correctly labeled data set.}
\label{fig:charpairs}
\end{figure}
