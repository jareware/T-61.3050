...
def optimize_parameters(vectors, labels):
    ...
    assert len(labels_distribution) > 1
...
def kfold(vectors, labels, count):
    ...
    assert len(clearx_splitted) == len(cleary_splitted) == len(x_splitted) == len(y_splitted) == count
...
def get_svm_parameters(vectors, labels, special_sets_values):
    ...
    for key in special_sets_values:
        ...
        for i in range(20, 1, -1):
             filename = "params%s-oneway-%s-%s.json" % (i, key, special_sets_values[key])
...
def predict_character(**kwargs):
    ...
    assert all(key in kwargs for key in ["data", "cleary", "clearx", "index", "svm", "special_svms", "special_sets_values"])
